...
{
    public static class DapperMySQL<T> where T : new()
    {
        public static MySqlConnection MySqlConnection()
        {
            var connection = new MySqlConnection(application.MX.DapperExtensions.connStringMySQL);
            connection.Open();
            return connection;
        }
        ...
        public static int AddData(T t, params object[] keys)
        {
            ...
            using (IDbConnection Conn = MySqlConnection())
            {
                return Conn.Query<int>(sql).ToList()[0];
            }
        }
        /// <summary>
        /// 删除指定行数据
