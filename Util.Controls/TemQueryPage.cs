...
{
    public partial class TemQueryPage : UserControl
    {
        public TemQueryPage()
        {
            InitializeComponent();
        ...
        private void buttQuery_Click(object sender, EventArgs e)
        {
            ...
            List<application.Model.MBakingTem> model = application.MX.DapperSQLServer<application.Model.MBakingTem>.QueryData(application.DAL.DataDAL.QueryTemData(this.combMac.SelectedIndex, ss, this.combSide.SelectedIndex , begin, end));
            if (model != null)
            {
